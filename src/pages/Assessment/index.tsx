...
export default function Assessment(){
  ...
  const [questions, setQuestions] = useState(Array.from({ length: 10 }, () => ({ question: "", options: ["","","",""], correctAnswer: 0 })));
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if(!snapshot.empty){
        ...
        const filled = [
          ...loaded,
          ...Array.from({ length: 10 - loaded.length }, () => ({ question: "", options: ["", "", "", ""], correctAnswer: 0 })),
        ];
        setQuestions(filled);
      }else{
        setQuestions(Array.from({ length: 10 }, () => ({ question: "", options: ["", "", "", ""], correctAnswer: 0 })));
      }
    }
  ...
  const handleSave = async () => {
    ...
    if(!snapshot.empty){
      ...
      await setDoc(doc(db, "avaliacao", docId), data);
    }else{
  ...
  return(
    <div className="flex flex-col min-h-screen gap-4 p-10">
      ...
      {!selected ? (
        ...
        <div className="bg-white p-6 rounded">
          <div className="flex justify-between items-center">
            <h3 className="text-blackColor1 text-xl font-medium font-Poppins mb-4">Avaliação sobre {selected}</h3>
      ...
      <Modal isOpen={showModal} onClose={handleClose} title="Salvo com sucesso!">
        <p className="font-Poppins text-blackColor1">Avaliação sobre <strong>{selected}</strong> foi salvo com sucesso!</p>
      </Modal>
    </div>
  )
}